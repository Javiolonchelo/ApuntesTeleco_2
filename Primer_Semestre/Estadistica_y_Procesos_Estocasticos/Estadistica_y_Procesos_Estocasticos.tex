...
\usepackage{lmodern}
\usepackage[utf8]{inputenc} % Escribir en cualquier idioma
\usepackage[greek,spanish,es-tabla,es-nodecimaldot,es-noindentfirst]{babel}

\usepackage{babelbib} % Para usar bibliografía en español.
...
\usepackage[thicklines]{cancel} % when something goes to zero
...
\usepackage{color}         % change the colour of the words
\usepackage{verbatim}      % display LaTeX commands instead of having them executed.
\usepackage{longtable}     % helps with long table options
\usepackage{enumerate}
\usepackage{epsf}          % old package handles encapsulated postscript issues
\usepackage{bm}            % special bold-math package. usge: \bm{mathsymbol}
\usepackage{textcomp} % provides many text symbols (such as baht, bullet, copyright, musical­note, onequarter, section and yen
\usepackage{gensymb} % Permite símbolo º
\usepackage{geometry} % Fijar los márgenes de todo el documento
\geometry{a4paper,left=35mm,right=35mm,top=15mm,bottom=15mm}

\usepackage{isotope} % Poner isótopos
\usepackage{maybemath} % Making math bold, upright, italic or sans-serif according to context
\usepackage{upgreek} % Letras griegas
\usepackage{wasysym} % Provides male, female and astronomical symbols
\usepackage[italic]{hepparticles} % Ver documentación adjunta
\usepackage{subdepth} % Equalise the height of subcripts in maths
\usepackage{physics} % Ver documentación adjunta
\usepackage{braket} % Notación Bra-ket de Dirac
\newcommand{\Int}{\int\limits} % símbolo de integrales y límites
\usepackage{tensor} % escribir tensores
\usepackage{chemformula} % reacciones y fórmulas químicas
\usepackage{tikz} % minipages
\usepackage{siunitx} % notacion científica
\usepackage{url} % enlaces a páginas web
\usepackage{multirow} % Necessary when placing two or more numbers in the same row
\usepackage{multicol} % Necessary when placing two or more numbers in the same column

\usepackage[colorlinks=true]{hyperref}  % fijar los colores de las referencias
\hypersetup{citecolor = blue, linkcolor = blue, urlcolor = blue} % colores de los enlaces a citas, figuras y url
...
\usepackage{siunitx} % Required for alignment of decimal numbers in the tables
\sisetup{ % How many digital places it should display
...
\newtheorem{formulario}{Formulario}
