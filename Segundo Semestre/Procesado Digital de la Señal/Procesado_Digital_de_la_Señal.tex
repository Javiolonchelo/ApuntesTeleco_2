...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[auto,>=latex']
		...
		\node [block, below of=input, node distance=0cm] (A) {ADC};

		\node [block, right of=A, node distance=2cm] (B) {$(N)\text{SDU}$};

		\node [text width=3.2cm, left of=A, node distance=3.5cm] (text6) {Metemos la información de control.};
...
Para trabajar con sistemas continuos y discretos, vamos a establecer unos criterios de \textbf{equivalencia}, que usaremos para relacionar ambos tipos de sistema.
