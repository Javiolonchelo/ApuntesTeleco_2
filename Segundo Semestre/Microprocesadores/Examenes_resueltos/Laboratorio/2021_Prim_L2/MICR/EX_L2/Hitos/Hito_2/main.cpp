...
int main (void) {
  ...
  int distancia_dm;                                 //almacena el valor medido por el ultrasonidos en decimetros(quizá se pueda resolver sin necesitarla)
  ...
  g_dsr.pulsewidth_us(g_lit.read_u16()*100/65535);  //pulsewidth que indica el brillo del display dependiendo de g_lit(LDR) estando encendido
  g_seven_seg = to_7seg(11);                        //muestra - en el display derecho
  ...
  for (;;) {
    ...
    if(b_timer_echo_fall){                          //si se produce flanco de bajada, ha terminado de medir
      ...
      distancia_dm = timer_echo.read_us()/580;      //obtenemos la distancia medida, y dividimos entre 58 para cm, entre 580 para dm
      if(distancia_dm < 4)                          //si distancia menor de 4dm
        cnt_near++;                                 //sumamos 1 a la variable que nos dicen
      if(distancia_dm > 9){                         //si distancia mayor de 9dm
        distancia_dm = 10;                          //distancia igual a 10 para que represente A
        g_seven_seg = ((g_dsl) ? to_7seg(distancia_dm) : to_7seg(11));  //representa A si esta encendido display izquierdo, - si esta encendido el display derecho
      }
      else
        g_seven_seg = ((g_dsl) ? to_7seg(distancia_dm) : to_7seg(11));  //representa la distancia si esta encendido el display izquierdo, - si esta encendido el display derecho
    }
    if(b_tick_mux){                                 //multiplexacion, entra cada 1ms
      b_tick_mux = false;                           //eliminamos el flag
      estado = !estado;                             //variable para la multiplexacion
      if(estado){
        g_dsl = 1;
        g_seven_seg = to_7seg(distancia_dm);
        g_dsr.pulsewidth_us(0);
      }
      else{
        g_dsl = 0;
        g_seven_seg = to_7seg(11);
        g_dsr.pulsewidth_us(g_lit.read_u16()*100/65535);
      }
    }
    __disable_irq();
