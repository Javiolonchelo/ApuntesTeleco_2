...
int main (void) {
  ...
  g_dsr.period_us(100);
  uint16_t brillo_dsr = 100;
  
  led.period_ms(100);
  led.pulsewidth_ms(100);
  ...
  for (;;) {
    ...
    if(tick_mux_event)
    {
      ...
      g_dsl = !g_dsl;
      g_dsl ? g_dsr.pulsewidth_us(0) : g_dsr.pulsewidth_us(brillo_dsr);
    ...
    if(tick_measure_event)
    {
      ...
      trig = 1;
      brillo_dsr = ldr.read_u16() * 100 / 65535;
      timeout_trig.attach_us(timeout_trig_isr, 10);
      led.pulsewidth_ms(100 - brillo_dsr);
    }
